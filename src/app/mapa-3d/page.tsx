...
// Dynamically import the Map component to avoid SSR issues with deck.gl
const Map3D = dynamic(
    () => import('../../components/Map/Map3D'),
    {
        ssr: false,
        loading: () => (
            <div className="w-full h-full flex items-center justify-center bg-zinc-900 text-white">
                <div className="animate-pulse flex flex-col items-center">
                    ...
                    <span className="text-sm text-zinc-400">Cargando mapa 3D...</span>
                </div>
            </div>
        )
    }
        ...
        <main className="flex min-h-screen flex-col items-center justify-between relative overflow-hidden">
            ...
            <div className="absolute top-4 left-4 z-[1000] bg-zinc-950/80 backdrop-blur-md border border-zinc-800 p-4 rounded-xl shadow-2xl max-w-sm">
                <h1 className="text-2xl font-bold bg-gradient-to-r from-cyan-400 to-purple-500 bg-clip-text text-transparent">
                    🧪 Mapa 3D Experimental
                </h1>
                <p className="text-xs text-zinc-400 mt-1">
                    Visualización 3D de densidad con hexágonos • Powered by deck.gl
                </p>
            </div>
